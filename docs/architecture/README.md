...
| --- | --- |
| `apps/*` | Named app workspaces. They currently export app names only and do not contain runnable UI code. |
...
There is no HTTP request path yet. The implemented flow is exercised through tests and direct TypeScript use cases.

1. Test or caller creates a work-management composition root.
2. `CreateWorkRequest` validates input through `WorkRequest.create`.
3. The in-memory repository stores the tenant-scoped request by `tenantId:id`.
4. The in-memory outbox records a `WorkRequestCreated` payload.
5. `QualifyWorkRequest` reads by tenant and request ID, checks the domain state transition, saves the updated request, and appends a `WorkRequestQualified` payload.
...
flowchart TD
	...
	Tests --> WMRoot[work-management composition root]
	...
	Tests --> Identity[identity-tenant assertTenantAccess]
