...
export class InMemoryWorkRequestRepository implements WorkRequestRepository {
  ...
  async save(request: WorkRequest): Promise<void> {
    this.items.set(`${request.props.tenantId}:${request.props.id}`, request);
  }
  async getById(tenantId: string, id: string): Promise<WorkRequest | undefined> {
    return this.items.get(`${tenantId}:${id}`);
  }
  async listByStatus(
    tenantId: string,
    status: WorkRequest['props']['status'],
  ): Promise<WorkRequest[]> {
    return [...this.items.values()].filter(
      (item) => item.props.tenantId === tenantId && item.props.status === status,
    );
  }
}
